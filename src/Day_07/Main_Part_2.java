...
public class Main_Part_2 {
    ...
    private static Map<Character, Integer> getCardCountMap(String handToCheck) {
        ...
        if (cardCountMap.containsKey('J') && cardCountMap.size() > 1) {

            int nJoker = cardCountMap.get('J');
            cardCountMap.remove('J');

            char strChar = '0';
            int freqChar = 0;

            for (Character cardChar : cardCountMap.keySet()) {

                int cardCharFrequency = cardCountMap.get(cardChar);

                if (cardCharFrequency > freqChar) {
                    strChar = cardChar;
                    freqChar = cardCharFrequency;
                    continue;
                }

                if (cardCharFrequency == freqChar) {

                    int strCharValue = cardStrengthMap.get(strChar);
                    int cardCharValue = cardStrengthMap.get(cardChar);

                    if (cardCharValue > strCharValue) strChar = cardChar;

                }

            }

            cardCountMap.put(strChar, cardCountMap.get(strChar) + nJoker);
